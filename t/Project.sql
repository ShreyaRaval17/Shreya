...
-- 1) Display all the information in the table emp and dept
...
-- 3) Display the emp name concated with the job id, separeted by comma and space, and name the column Employee and title

select 
	concat_ws(' , ',first_name,last_name,job_id) as Employee_and_title 
from employees;
...
-- 5) Create a query to display all the data from emp table. separate each column by a comma. name the the column THE OUTPUT

select 
	concat_ws(", ",employee_id,
	first_name,
	last_name,
	email,
	phone_number,
...
-- 6)Display the names and salries of all employees with a salary greater than 2000
...
-- 7) Display the names and date of employees with the column headers "NAME" and "Start_date"
...
-- 8) Display the names and hire date of all employees in the order they were hired

select 
	concat(first_name,last_name) as Name_,
	hire_date 
from employees 
...
-- 9) Display the names and saleries of all employees in reverse salary order
...
-- 10) Display 'emp name' and 'dept no' who are all earned commission and display salary in reverse order
...
select 
	department_id,
	sum(salary) as total_salary_of_employee 
from employees
...
SELECT 
	concat(first_name," ",last_name) as employee_name, 
	salary, 
	IFNULL(commission_pct * 2, 'No commission') AS commission
FROM employees;
...
select 
	e1.first_name, e1.Department_id
...
select 
	sum(salary) as salary_of_employee,
	manager_id 
from employees
...
-- we don't have Managers name in our data
...
select 
concat(
ucase(substring(last_name,2,1)),
lcase(substring(last_name,3,5))
) 
as employee_name 
from employees;
...
select 
	concat(first_name,"-",last_name) as employee_anme,
...
-- 4. Display the employee ID by Appending two zeros after 2nd digit and 'E' in the end, department id, salary and the manager name all in Upper case, if the first name consists of 'z' replace it with '$! 

SELECT CONCAT(LEFT(employee_id, 2), '00', RIGHT(employee_id, LENGTH(employee_id) - 2), 'E') AS new_emp_id,
       department_id,
       salary,
       REPLACE(UPPER(first_name), 'A', '$!') AS ename
	FROM employees;
...
-- 5. Write a query that displays the employee's last names with the first letter capitalized and all other letters lowercase, and the length of the names, for all employees whose name starts with J, A, or M. Give each column an appropriate label. 
--   Sort the results by the employees' last names 

select 
	concat(ucase(mid(first_name,1,1)),lcase(mid(first_name,2))) as name,
...
select 
	last_name, concat('$',LPAD(FORMAT(salary,0),15,'$')) as SALARY 
from employees;
...
select concat(UCASE(substring(first_name,1,1)),LCASE(substring(first_name,2))) as initacaps from employees;
...
select 
	substring(street_address,position(' ' in street_address ),position(' ' in street_address )) as sub_string 
from locations;
...
select first_name, LCASE(concat_ws('',substring(first_name,1,1),last_name,'@systechusa.com')) as email_address from employees;
...
select 
	concat(e.first_name," ", e.last_name) as Name, d.department_name 
from employees as e 
...
select 
	e.job_id, d.department_name, l.city as location 
...
-- we don't have manager's last name in our tables 
-- so, if it's given then our query will be

select last_name, employee_id, manager_id, manager_name from employees
...
select last_name, department_id, count(*) from employees 
...
select 
	...
	j.job_title as Job, d.department_name, e.salary,
	case
...
-- we dont have manager name in our data, if it was given then our query will be

select 
	concat(e.first_name," ",e.last_name) as Employee_name, e.hire_date as emp_hire_date, m.manager_name,m.m_hire_date as man_hire_date
from employees as e
join employees as m
...
-- we don't have enough data to write this query.
...
-- Employees in department 1 and 3 are given a 5% raise, employees in department 2 are given a 10% raise, employees in departments 4 and 5 are given a 15% raise, and employees in department 6 are not given a raise. 

-- we don't have department no in our data, if it was given then our query will be 

select employee_id,salary,
    case
     when department_no in (1,3) then salary*1.05
     when department_no= 2 then salary*1.10
	 when department_no in (4,5) then salary*1.15
     when department_no= 6 then salary
     else salary
...
select e.manager_name,e.salary,d.department_name 
from  employees as e
join departments as d
on e.department_id=d.department_id
order by salary desc
